...
impl ProjectionHandler<u64, u64> for Counter {
    fn projection_id(&self) -> &str { "counter-v1" }
...
fn event(stream: EntityId, sequence: u64, value: u64) -> StoredEvent<u64> {
    StoredEvent {
        ...
        envelope: EventEnvelope::new(
            ...
            EntityId::new(), TimestampMs::new(sequence),
            SequenceNumber::new(sequence), value,
        ).unwrap(),
    }
}
...
fn projection_runtime_replays_safely_and_registry_bootstraps() {
    ...
    registry.register(ProjectionDescriptor::new("counter-v1", 1, "counter projection")).unwrap();
    assert!(registry.get("counter-v1").is_some());

    let stream = EntityId::new();
    let mut runtime = ProjectionRuntime::new(stream, SequenceNumber::ZERO, EventId::new(), 0, Counter).unwrap();
