...
/// Processing phase of a projection cursor.
...
/// Durable cursor for an idempotent projection.
...
impl ProjectionCheckpoint {
    ...
    pub fn advance(&mut self, next: &Self) -> KernelResult<()> {
        ...
        if next.sequence == self.sequence && next.event_id != self.event_id {
            return Err(KernelError::InvalidInput(
                "projection checkpoint cannot replace an event at the same sequence".into(),
            ));
        }
    ...
    pub fn is_caught_up(&self, target: SequenceNumber) -> bool {
