...
impl IdempotencyKey {
    pub fn new(value: impl Into<String>) -> KernelResult<Self> {
        ...
        if value.trim().is_empty() {
            return Err(KernelError::InvalidIdentifier("idempotency key must not be empty".to_owned()));
        }
        Ok(Self(value))
    }
...
impl IdempotencyLedger {
    ...
    pub fn record(&mut self, key: IdempotencyKey, receipt: IdempotencyReceipt) -> KernelResult<IdempotencyReceipt> {
        if let Some(existing) = self.receipts.get(&key).copied() { return Ok(existing); }
...
mod tests {
    ...
    #[test]
    fn rejects_empty_keys() { assert!(IdempotencyKey::new("  ").is_err()); }
    ...
    fn returns_the_original_receipt_for_duplicate_keys() {
        ...
        let first = IdempotencyReceipt { event_id: EventId::new(), sequence: SequenceNumber::new(1) };
        let second = IdempotencyReceipt { event_id: EventId::new(), sequence: SequenceNumber::new(2) };
