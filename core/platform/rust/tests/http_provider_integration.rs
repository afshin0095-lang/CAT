...
impl FixtureServer {
    fn start(fail_remaining: usize, slow: bool, unhealthy: bool) -> Self {
        ...
        let state = Arc::new(Mutex::new(FixtureState { fail_remaining, slow, unhealthy, ..Default::default() }));
        ...
        Self { address, state, join: Some(join) }
    }

    fn requests(&self) -> usize { self.state.lock().unwrap().requests }
}
...
fn handle(stream: &mut TcpStream, state: &Arc<Mutex<FixtureState>>) {
    ...
    let (status, body, slow) = {
        ...
        state.requests += 1;
        if path == "/health" {
            if state.unhealthy { (503, json!({"status":"degraded"}), false) }
            else { (200, json!({"status":"ready"}), false) }
        } else if path == "/fail" {
            if state.fail_remaining > 0 { state.fail_remaining -= 1; (503, json!({"error":"temporary"}), false) }
            else { (200, json!({"ok":true}), false) }
        } else if path == "/slow" {
    ...
    if slow { thread::sleep(Duration::from_millis(150)); }
    let payload = serde_json::to_string(&body).unwrap();
    let reason = match status { 200 => "OK", 503 => "Service Unavailable", 404 => "Not Found", _ => "Error" };
...
fn request(operation: &str) -> AdapterRequest {
    AdapterRequest::from_command(
        IntegrationCommand::new(IntegrationTarget::Llm, operation, IntegrationContext::new("platform-http-fixture")),
        json!({"prompt":"fixture"}),
    )
}
...
fn http_provider_classifies_non_2xx_as_transport_failure() {
    let server = FixtureServer::start(0, false, false);
    let adapter = HttpJsonProviderAdapter::new("fixture", IntegrationTarget::Llm, &server.address, ["fail"], "/health", Duration::from_secs(1)).unwrap();
    let error = adapter.execute(&request("fail")).unwrap_err();
    assert!(matches!(error, PlatformError::TransportUnavailable(message) if message.contains("503")));
}

#[test]
fn http_provider_timeout_is_bounded() {
    let server = FixtureServer::start(0, true, false);
    let adapter = HttpJsonProviderAdapter::new("fixture", IntegrationTarget::Llm, &server.address, ["slow"], "/health", Duration::from_millis(25)).unwrap();
...
fn resilient_provider_retries_and_recovers_after_fixture_failure_window() {
    ...
    assert!(response.accepted);
    assert_eq!(resilient.circuit_state(), cat_platform::ProviderCircuitState::Closed);
...
fn resilient_provider_opens_circuit_after_repeated_fixture_failures() {
    ...
    let resilient = ResilientProviderAdapter::new(
        Arc::new(adapter),
        ProviderRetryConfig { max_attempts: 3, retry_delay: Duration::ZERO, max_retry_delay: Duration::ZERO },
        ProviderCircuitConfig { failure_threshold: 2, recovery_after: Duration::from_secs(60) },
    ).unwrap();
    assert!(resilient.execute(&request("fail")).is_err());
    assert_eq!(resilient.circuit_state(), cat_platform::ProviderCircuitState::Open);
    assert!(resilient.execute(&request("fail")).is_err());
