...
impl ProviderFailureClass {
    pub const fn retryable(self) -> bool {
        matches!(
            self,
            Self::RateLimited | Self::Server | Self::Timeout | Self::Transport
        )
    }
...
impl ProviderFailure {
    ...
    pub fn transport(operation: impl Into<String>, message: impl Into<String>) -> Self {
        Self {
            class: ProviderFailureClass::Transport,
            status_code: None,
            operation: Some(operation.into()),
            provider_id: None,
            message: message.into(),
...
mod tests {
    ...
    fn classifies_http_failures_deterministically() {
        assert_eq!(
            ProviderFailure::http(StatusCode::TOO_MANY_REQUESTS, "x", "busy").class,
            ProviderFailureClass::RateLimited
        );
        assert!(
            ProviderFailure::http(StatusCode::SERVICE_UNAVAILABLE, "x", "down")
                .class
                .retryable()
        );
        assert!(
            ProviderFailure::http(StatusCode::UNAUTHORIZED, "x", "bad key")
                .class
                .terminal()
        );
    }
