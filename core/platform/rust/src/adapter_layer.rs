...
/// business semantics: domain behavior remains in the owning core, while this layer selects the
/// owning runtime, validates the adapter boundary, and preserves request context.
#[derive(Default)]
pub struct PlatformAdapterLayer {
...
impl PlatformAdapterLayer {
    ...
    pub fn execute(&mut self, command: TypedCoreCommand) -> PlatformResult<TypedCoreResponse> {
        match command.target() {
            ...
            | IntegrationTarget::Memory => self.concrete.execute(command),
            IntegrationTarget::Llm
            | IntegrationTarget::Reasoning
            | IntegrationTarget::Decision
            | IntegrationTarget::Retrieval => self.remaining.execute(command),
            IntegrationTarget::Planning | IntegrationTarget::Orchestrator => {
                self.execute_workflow(command)
            }
        }
    }
    ...
    fn execute_workflow(&mut self, command: TypedCoreCommand) -> PlatformResult<TypedCoreResponse> {
        ...
        let core = match command {
            TypedCoreCommand::Planning(core) | TypedCoreCommand::Orchestrator(core) => core,
            _ => unreachable!("target was checked before workflow dispatch"),
        };

        match (target, core.operation.as_str()) {
            (IntegrationTarget::Planning, "validate_plan") => {
                let plan: cat_planning::Plan = serde_json::from_value(core.payload)
                    .map_err(|error| invalid(format!("invalid plan payload: {error}")))?;
            ...
            (IntegrationTarget::Orchestrator, "schedule") => {
                let request: ScheduleCommand = serde_json::from_value(core.payload)
                    .map_err(|error| invalid(format!("invalid schedule payload: {error}")))?;
                self.workflow.schedule(
                    &core.context,
                    request.workflow_id,
                ...
                Ok(response(
                    ...
                    json!({
                        ...
                        "queued": true,
                        "queue_depth": self.workflow.queue_depth(),
                        "correlation_id": core.context.correlation_id,
                        "causation_id": core.context.causation_id,
                    }),
                ))
            }
            (IntegrationTarget::Orchestrator, "status") => Ok(response(
                core.context.request_id,
                target,
                "status",
                json!({
            ...
            _ => Err(invalid(format!(
                "unsupported workflow adapter operation: {:?}/{}",
...
mod tests {
    ...
    fn routes_stateful_cores_to_concrete_runtime() {
        ...
        let error = layer.execute(command).unwrap_err();
        assert!(matches!(error, PlatformError::InvalidCommand(_)));
    }

    #[test]
    fn routes_ai_cores_to_remaining_runtime() {
        let mut layer = PlatformAdapterLayer::new();
        let command = TypedCoreCommand::Llm(CoreCommand::new(
            Uuid::now_v7(),
            "health",
            IntegrationContext::new("adapter-test"),
            Value::Null,
        ));
        let response = layer.execute(command).unwrap();
        assert_eq!(response.target, IntegrationTarget::Llm);
        assert_eq!(response.payload["ready"], true);
    }

    #[test]
    fn planning_adapter_validates_through_planning_owner() {
        let mut builder = PlanBuilder::new("adapter test");
        builder = builder.metadata("domain", json!("affiliate"));
        let _ = builder.step("approve", StepKind::Approval);
        let plan = builder.build();

        let context = IntegrationContext::new("adapter-test").with_workflow(Uuid::now_v7());
        let mut layer = PlatformAdapterLayer::new();
        ...
        assert_eq!(response.payload["validated"], true);
    }

    #[test]
    fn orchestrator_adapter_preserves_workflow_identity() {
        let workflow_id = Uuid::now_v7();
        let context = IntegrationContext::new("adapter-test").with_workflow(workflow_id);
        let mut layer = PlatformAdapterLayer::new();
        let command = TypedCoreCommand::Orchestrator(CoreCommand::new(
            Uuid::now_v7(),
            "schedule",
            context,
            json!({"workflow_id": workflow_id, "not_before_ms": 100, "priority": 7}),
        ));

        let response = layer.execute(command).unwrap();
        assert_eq!(response.payload["workflow_id"], json!(workflow_id));
        assert_eq!(response.payload["queued"], true);
        assert_eq!(layer.queue_depth(), 1);
    }

    #[test]
    fn wrong_operation_is_rejected_at_adapter_boundary() {
        let mut layer = PlatformAdapterLayer::new();
        let command = TypedCoreCommand::Retrieval(CoreCommand::new(
            Uuid::now_v7(),
            "delete_everything",
            IntegrationContext::new("adapter-test"),
            json!({}),
