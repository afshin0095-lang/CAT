use crate::{
    ...
    ProviderCapabilities, ProviderCircuitBreaker, ProviderCircuitConfig, ProviderHealth,
    ProviderHealthProbe,
};
use std::sync::Arc;
use std::thread;
...
impl ProviderRetryConfig {
    pub fn validate(self) -> PlatformResult<Self> {
        if self.max_attempts == 0 {
            return Err(PlatformError::InvalidCommand(
                "provider retry max_attempts must be greater than zero".into(),
            ));
        }
        if self.max_retry_delay < self.retry_delay {
            return Err(PlatformError::InvalidCommand(
                "provider retry max_retry_delay must be >= retry_delay".into(),
            ));
        }
        Ok(self)
    }
...
impl ResilientProviderAdapter {
    pub fn new(
        ...
        Ok(Self {
            circuit: ProviderCircuitBreaker::new(adapter, circuit)?,
    ...
    pub fn circuit_state(&self) -> crate::ProviderCircuitState {
        self.circuit.state()
    }
    pub fn consecutive_failures(&self) -> u32 {
        self.circuit.consecutive_failures()
    }
    pub fn retry_config(&self) -> ProviderRetryConfig {
        self.retry
    }
    ...
    pub fn execute(&self, request: &AdapterRequest) -> PlatformResult<AdapterResponse> {
        ...
        for attempt in 1..=self.retry.max_attempts {
            match self.circuit.execute(request) {
                Ok(response) => return Ok(response),
                Err(PlatformError::TransportUnavailable(message))
                    if message.contains("circuit is open")
                        || message.contains("probe already in progress") =>
                {
                    return Err(PlatformError::TransportUnavailable(message));
                }
                Err(error) => {
        ...
        Err(last_error.unwrap_or_else(|| {
            PlatformError::TransportUnavailable("provider execution failed without an error".into())
        }))
    }
}
...
impl ExternalProviderAdapter for ResilientProviderAdapter {
    fn capabilities(&self) -> ProviderCapabilities {
        ResilientProviderAdapter::capabilities(self)
    }
    fn execute(&self, request: &AdapterRequest) -> PlatformResult<AdapterResponse> {
        ResilientProviderAdapter::execute(self, request)
    }
    fn probe_health(&self) -> PlatformResult<ProviderHealth> {
...
mod tests {
    use super::*;
    use crate::{
        AdapterRequest, DeterministicProviderAdapter, IntegrationCommand, IntegrationContext,
        IntegrationTarget, PlatformError, ProviderHealth, ProviderId,
    };
    use serde_json::json;
    ...
    impl ExternalProviderAdapter for FlakyAdapter {
        fn capabilities(&self) -> ProviderCapabilities {
        ...
        fn execute(&self, request: &AdapterRequest) -> PlatformResult<AdapterResponse> {
            ...
            Ok(AdapterResponse {
                ...
                accepted: true,
                payload: json!({"provider":"flaky","attempt":"success"}),
            })
        }
    }
    ...
    fn request() -> AdapterRequest {
        AdapterRequest::from_command(
            IntegrationCommand::new(
                ...
                "generate",
                IntegrationContext::new("resilience-test"),
            ),
            json!({"prompt":"ping"}),
        )
    }
