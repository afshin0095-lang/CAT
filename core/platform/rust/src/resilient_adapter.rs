...
impl ResilientProviderAdapter {
    pub fn new(adapter: Arc<dyn ExternalProviderAdapter>, retry: ProviderRetryConfig, circuit: ProviderCircuitConfig) -> PlatformResult<Self> {
    ...
    pub fn execute(&self, request: &AdapterRequest) -> PlatformResult<AdapterResponse> {
        ...
        for attempt in 1..=self.retry.max_attempts {
            match self.circuit.execute(request) {
                ...
                Err(PlatformError::TransportUnavailable(message)) if message.contains("circuit is open") || message.contains("probe already in progress") => {
        ...
        Err(last_error.unwrap_or_else(|| PlatformError::TransportUnavailable("provider execution failed without an error".into())))
    }
}
        ...
        fn new(failures_before_success: u32) -> Self {
            Self {
                capabilities: ProviderCapabilities {
                    provider_id: ProviderId::new("flaky").unwrap(), target: IntegrationTarget::Llm,
                    operations: vec!["generate".into()], health: ProviderHealth::Ready,
                },
    ...
    impl ExternalProviderAdapter for FlakyAdapter {
        ...
        fn execute(&self, request: &AdapterRequest) -> PlatformResult<AdapterResponse> {
            if self.failures_before_success.load(Ordering::SeqCst) > 0 {
                ...
                return Err(PlatformError::TransportUnavailable("temporary provider failure".into()));
            }
            Ok(AdapterResponse { request_id: request.context.request_id, target: request.target, operation: request.operation.clone(), accepted: true, payload: json!({"provider":"flaky","attempt":"success"}) })
        }
    }

    fn request() -> AdapterRequest {
        AdapterRequest::from_command(
            IntegrationCommand::new(IntegrationTarget::Llm, "generate", IntegrationContext::new("resilience-test")),
            json!({"prompt":"ping"}),
        )
    }
