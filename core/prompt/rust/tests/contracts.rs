...
fn document() -> PromptDocument {
    ...
    document.push_block(PromptBlock::new(PromptRole::System, "bootstrap", "You are CAT. Mission={{mission}}."));
    document.push_block(PromptBlock::new(PromptRole::User, "request", "Execute task {{task}}."));
...
fn sensitive_variable_is_denied_by_default() {
    ...
    assert!(PromptRenderer::render(&document, &PromptPolicy::new(["mission", "task", "api_key"])).is_err());
}
...
fn missing_variable_is_explicit() {
    ...
    assert!(matches!(PromptRenderer::render(&document, &PromptPolicy::new(std::iter::empty::<&str>())),
