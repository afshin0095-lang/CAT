...
impl ExecutionContext {
    ...
    pub fn checkpoint(&mut self, now_ms: u64) -> Result<(), ExecutionError> {
        if self.state == ExecutionState::Cancelled || self.cancellation.is_cancelled() { self.state = ExecutionState::Cancelled; return Err(ExecutionError::Cancelled); }
        if self.deadline_ms.is_some_and(|deadline| now_ms >= deadline) { self.state = ExecutionState::TimedOut; return Err(ExecutionError::TimedOut); }
        Ok(())
    }
    ...
    pub fn is_terminal(&self) -> bool { matches!(self.state, ExecutionState::Completed | ExecutionState::Cancelled | ExecutionState::TimedOut) }
}
