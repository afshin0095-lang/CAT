...
fn execution_context_rejects_expired_lease() {
    ...
    let result = ExecutionContext::start(TaskId::new(), 1, &lease, lease.expires_at_ms, None);
...
fn execution_context_cancels_and_propagates_to_child_token() {
    let lease = TaskLease::acquire("worker-a", 1, Duration::from_secs(10)).unwrap();
    let mut context = ExecutionContext::start(TaskId::new(), 1, &lease, 0, None).unwrap();
...
fn execution_context_times_out_at_deadline() {
    let lease = TaskLease::acquire("worker-a", 1, Duration::from_secs(10)).unwrap();
    let mut context = ExecutionContext::start(TaskId::new(), 1, &lease, 100, Some(Duration::from_millis(50))).unwrap();
