...
impl KnowledgeGraph {
    ...
    pub fn insert_edge(&mut self, edge: KnowledgeEdge) -> KnowledgeStoreResult<KnowledgeEdgeId> {
        if !self.nodes.contains_key(&edge.from) {
            return Err(KnowledgeStoreError::NodeNotFound(edge.from));
        }
    ...
    pub fn find_node(&self, entity_type: &str, canonical_key: &str) -> Option<&KnowledgeNode> {
        self.canonical_index
            .get(&(entity_type.to_owned(), canonical_key.to_owned()))
            .and_then(|id| self.nodes.get(id))
    }

    pub fn edges_from(&self, node: KnowledgeNodeId) -> Vec<&KnowledgeEdge> {
        self.edges.values().filter(|edge| edge.from == node).collect()
    }

    pub fn edges_to(&self, node: KnowledgeNodeId) -> Vec<&KnowledgeEdge> {
        self.edges.values().filter(|edge| edge.to == node).collect()
    }

    pub fn related(&self, node: KnowledgeNodeId, relation: &str) -> Vec<KnowledgeNodeId> {
        self.edges.values()
            .filter(|edge| edge.from == node && edge.relation == relation)
            .map(|edge| edge.to)
            .collect()
    }

    pub fn relation_types(&self) -> BTreeSet<String> {
        self.edges.values().map(|edge| edge.relation.clone()).collect()
    }
