...
pub fn validate_evidence(evidence: &EvidenceRef) -> Vec<KnowledgeViolation> {
    ...
    if evidence.source.trim().is_empty() {
        violations.push(KnowledgeViolation::InvalidEvidenceSource);
    }
    if evidence.reference.trim().is_empty() {
        violations.push(KnowledgeViolation::InvalidEvidenceReference);
    }
    violations
}
...
pub fn validate_node(node: &KnowledgeNode) -> Vec<KnowledgeViolation> {
    ...
    for evidence in &node.evidence {
        violations.extend(validate_evidence(evidence));
    }
    violations
}
...
impl KnowledgeGraph {
    pub fn validate_consistency(&self) -> Vec<KnowledgeViolation> {
        ...
        for node in self.nodes_iter() {
            violations.extend(validate_node(node));
        }
        for edge in self.edges_iter() {
            if self.node(edge.from).is_none() || self.node(edge.to).is_none() {
                violations.push(KnowledgeViolation::MissingEndpoint);
            }
