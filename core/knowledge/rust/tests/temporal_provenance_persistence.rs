use cat_knowledge::{
    EvidenceRef, KnowledgeGraph, KnowledgeNode, KnowledgeSnapshotStore, MemorySnapshotStore,
    ProvenanceChain, ValidityWindow,
};
...
fn snapshot_persistence_does_not_change_graph_identity() {
    ...
    let restored = KnowledgeGraph::restore_from(&store)
        .expect("restore")
        .expect("snapshot");
