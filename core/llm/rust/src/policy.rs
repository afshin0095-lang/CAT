...
impl LlmRouter {
    ...
    pub async fn generate_stream(
        ...
        let route = self
            ...
            .resolve(Some(&request.model), request.safety)
            .ok_or(LlmError::NoRoute)?;
        self.provider_for(&route.provider)?
    ...
    pub async fn generate_default_stream(
        &self,
        mut request: GenerationRequest,
    ) -> Result<LlmGenerationStream, LlmError> {
        let route = self
    ...
    pub async fn generate_default_stream_resilient(
        &self,
        mut request: GenerationRequest,
    ) -> Result<LlmGenerationStream, LlmError> {
        let route = self
...
mod tests {
    use super::*;
    use crate::{DeterministicProvider, Message, collect_stream};
    ...
    async fn router_executes_only_through_the_policy_selected_provider() {
        ...
        let response = router
            .generate(request(SafetyClass::Standard))
            .await
            .unwrap();
    ...
    async fn router_rejects_safety_class_above_route_allowance() {
        ...
        assert!(matches!(
            router
                .generate(request(SafetyClass::Restricted))
                .await
                .unwrap_err(),
            LlmError::NoRoute
        ));
    ...
    async fn resilient_streaming_marks_provider_healthy_only_at_terminal_chunk() {
        ...
        let stream = router
            .generate_stream_resilient(request(SafetyClass::Standard))
            .await
            .unwrap();
        assert_eq!(
            router.health().snapshot(&provider).state,
            crate::ProviderHealthState::Healthy
        );
