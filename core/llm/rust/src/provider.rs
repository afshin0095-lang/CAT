...
pub trait LlmProvider: Send + Sync {
    ...
    async fn generate_stream(&self, request: GenerationRequest) -> Result<LlmGenerationStream, LlmError> {
...
impl LlmProvider for DeterministicProvider {
    ...
    async fn generate_stream(&self, request: GenerationRequest) -> Result<LlmGenerationStream, LlmError> {
        let response = self.generate(request).await?;
        let words: Vec<String> = response.content.split_whitespace().map(ToOwned::to_owned).collect();
        let last = words.len().saturating_sub(1);
        let chunks = words.into_iter().enumerate().map(move |(index, word)| {
            let delta = if index == last { word } else { format!("{word} ") };
