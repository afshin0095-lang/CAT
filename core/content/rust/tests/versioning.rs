use cat_content::{
    ContentDomain, ContentDomainError, ContentKind, ContentRepository, ContentStatus,
...
fn revision_creates_new_identity_and_preserves_source_version() {
    ...
    let original =
        ContentDomain::create(&mut repo, ContentKind::Article, "v1", "body v1", 1).unwrap();
...
fn revision_rejects_empty_payloads_without_mutating_source() {
    let mut repo = InMemoryContentRepository::default();
    let original =
        ContentDomain::create(&mut repo, ContentKind::Article, "stable", "body", 1).unwrap();
