...
fn content_lifecycle_is_guarded() {
    ...
    assert!(matches!(
        ...
        Err(ContentDomainError::InvalidState(_))
    ));
    assert_eq!(
    ...
    assert_eq!(
        ContentDomain::approve(&mut repo, record.id).unwrap().status,
        ContentStatus::Approved
    );
    assert_eq!(
        ContentDomain::publish(&mut repo, record.id).unwrap().status,
...
fn canonical_content_requires_real_fields() {
    let mut repo = InMemoryContentRepository::default();
    assert_eq!(
        ContentDomain::create(&mut repo, ContentKind::Article, "", "body", 1),
        Err(ContentDomainError::EmptyTitle)
    );
    assert_eq!(
        ContentDomain::create(&mut repo, ContentKind::Article, "title", "", 1),
