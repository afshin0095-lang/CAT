use cat_content::{ContentDomain, ContentDomainError, ContentKind, ContentStatus, InMemoryContentRepository, PublicationPolicy};
...
fn publish_with_policy_requires_approval_and_persists_published_state() {
    ...
    let published = ContentDomain::publish_with_policy(
        &mut repo,
        record.id,
