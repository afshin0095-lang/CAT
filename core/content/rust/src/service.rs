...
impl ContentDomain {
    pub fn create<R: ContentRepository>(
        repo: &mut R,
        kind: ContentKind,
    ...
    pub fn revise<R: ContentRepository>(
        repo: &mut R,
        id: crate::ContentId,
        title: impl Into<String>,
        body: impl Into<String>,
    ...
    pub fn submit_for_review<R: ContentRepository>(
        repo: &mut R,
        id: crate::ContentId,
    ) -> ContentDomainResult<ContentRecord> {
        let mut record = repo.get(id)?;
        if record.status != ContentStatus::Draft {
            return Err(ContentDomainError::InvalidState("review requires draft"));
        }
    ...
    pub fn approve<R: ContentRepository>(
        repo: &mut R,
        id: crate::ContentId,
    ) -> ContentDomainResult<ContentRecord> {
        let mut record = repo.get(id)?;
        if record.status != ContentStatus::Review {
            return Err(ContentDomainError::InvalidState("approval requires review"));
        }
    ...
    pub fn publish<R: ContentRepository>(
        repo: &mut R,
        id: crate::ContentId,
    ) -> ContentDomainResult<ContentRecord> {
    ...
    /// Records an observed publication outcome without changing authorization
    /// or canonical content truth.
