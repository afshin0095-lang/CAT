...
pub fn validate_plan(plan: &Plan) -> PlanValidationReport {
    ...
    if plan.goal.trim().is_empty() {
        report.errors.push(PlanValidationError::EmptyGoal);
    }
    if plan.steps.is_empty() {
        report.errors.push(PlanValidationError::EmptyPlan);
        return report;
    }
    ...
    for step in &plan.steps {
        if !ids.insert(step.id) {
            report
                .errors
                .push(PlanValidationError::DuplicateStep(step.id));
        }
    ...
    if actual != expected {
        report.errors.push(PlanValidationError::NonContiguousOrder);
    }

    for step in &plan.steps {
        for dep in &step.dependencies {
            if !ids.contains(&dep.prerequisite) {
                report.errors.push(PlanValidationError::UnknownDependency {
                    step: step.id,
    ...
    fn visit(
        ...
        if !visiting.insert(id) {
            errors.push(PlanValidationError::DependencyCycle(id));
            return;
        }
        if let Some(step) = plan.steps.iter().find(|s| s.id == id) {
            for dep in &step.dependencies {
                visit(dep.prerequisite, plan, visiting, visited, errors);
            }
        }
        visiting.remove(&id);
        visited.insert(id);
