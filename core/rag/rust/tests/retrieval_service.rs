use cat_rag::{
    DocumentChunk, DocumentIndex, Embedder, Embedding, InMemoryIndex, RagError, RetrievalService,
};
use serde_json::json;
use uuid::Uuid;
...
fn service_embeds_and_retrieves_with_tenant_isolation() {
    ...
    let hits = service
        .retrieve(tenant, "affiliate attribution", 5, Some(0.5))
        .unwrap();
...
fn service_rejects_non_finite_score_thresholds() {
    let service = RetrievalService::new(StaticEmbedder, InMemoryIndex::default());
    let error = service
        .retrieve(Uuid::now_v7(), "query", 5, Some(f32::NAN))
        .unwrap_err();
...
fn service_rejects_zero_limit() {
    let service = RetrievalService::new(StaticEmbedder, InMemoryIndex::default());
    let error = service
        .retrieve(Uuid::now_v7(), "query", 0, None)
        .unwrap_err();
