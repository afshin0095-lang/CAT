use crate::{EventBusError, EventBusResult, EventEnvelope, EventTransport, TransportId, TransportRegistry};
...
impl TransportRoute {
    pub fn exact_event(event_type: impl Into<String>, transport_id: TransportId) -> Self {
        Self { event_type: Some(event_type.into()), producer_prefix: None, transport_id }
    }

    pub fn producer_prefix(prefix: impl Into<String>, transport_id: TransportId) -> Self {
        Self { event_type: None, producer_prefix: Some(prefix.into()), transport_id }
    }

    fn matches(&self, event: &EventEnvelope) -> bool {
        let event_match = self.event_type.as_ref().map(|t| t == &event.event_type).unwrap_or(true);
        let producer_match = self.producer_prefix.as_ref().map(|p| event.producer.starts_with(p)).unwrap_or(true);
...
impl<T> EventRouter<T> {
    pub fn new(registry: TransportRegistry<T>) -> Self { Self { registry, routes: Vec::new() } }

    pub fn add_route(&mut self, route: TransportRoute) { self.routes.push(route); }

    pub fn route(&self, event: &EventEnvelope) -> EventBusResult<&TransportRoute> {
        self.routes.iter().find(|route| route.matches(event)).ok_or_else(|| {
            EventBusError::NoTransportRoute(event.event_type.clone())
        })
    }
}
...
mod tests {
    ...
    fn exact_event_route_wins_deterministically() {
        ...
        registry.register(EndpointTransport::new(TransportEndpoint::new(kafka.clone(), "cat."), RecordingTransport::default())).unwrap();
        let mut router = EventRouter::new(registry);
        router.add_route(TransportRoute::exact_event("affiliate.conversion", kafka.clone()));
        assert_eq!(router.route(&event("affiliate.conversion", "affiliate")).unwrap().transport_id, kafka);
    }
}
