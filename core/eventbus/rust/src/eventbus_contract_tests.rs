...
use crate::{
    ...
    EventRegistry, IdempotencyStore, InMemoryIdempotency, InMemoryInbox, InMemoryOutbox,
...
fn typed_event_creates_stable_contract_metadata() {
    let envelope = AffiliateCreated {
        affiliate_id: "aff-1".into(),
    }
    .into_envelope("affiliate-core")
    .unwrap();
...
fn registry_rejects_duplicate_and_unknown_versions() {
    ...
    assert!(matches!(
        ...
        Err(EventBusError::ContractConflict { .. })
    ));
    assert!(registry.require("affiliate.created", 1).is_ok());
    assert!(matches!(
        registry.require("affiliate.created", 2),
