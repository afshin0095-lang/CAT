...
impl<T> EndpointTransport<T> {
    pub fn new(endpoint: TransportEndpoint, inner: T) -> Self {
        Self { endpoint, inner }
    }
}
...
impl<T: EventTransport> EventTransport for EndpointTransport<T> {
    fn publish(&mut self, event: &EventEnvelope) -> EventBusResult<()> {
        if self.endpoint.health == TransportHealth::Unavailable {
            return Err(EventBusError::TransportUnavailable(
                self.endpoint.id.as_str().to_owned(),
            ));
        }
...
impl<T> TransportRegistry<T> {
    ...
    pub fn get(&self, id: &TransportId) -> Option<&EndpointTransport<T>> {
        self.endpoints.get(id)
    }
    pub fn get_mut(&mut self, id: &TransportId) -> Option<&mut EndpointTransport<T>> {
