...
fn envelope_contains_stable_contract_metadata() {
    let envelope = CampaignStarted {
        campaign_id: "cmp-001".into(),
    }
    .into_envelope("affiliate-engine")
    .unwrap()
    ...
    assert!(envelope.event_id.get_version_num() == Some(7));
}
...
fn handlers_run_in_registration_order() {
    ...
    for marker in [1_u8, 2_u8, 3_u8] {
        let observed = Arc::clone(&observed);
        bus.subscribe(
            CampaignStarted::TYPE,
            Arc::new(move |_| {
                observed.lock().unwrap().push(marker);
                Ok(())
            }),
        )
        .unwrap();
    ...
    let envelope = CampaignStarted { campaign_id: "cmp-002".into() }
        .into_envelope("affiliate-engine")
        .unwrap();

    let outcome = bus.publish(envelope).unwrap();
    assert_eq!(outcome, PublishOutcome::Published { handlers_called: 3 });
...
fn duplicate_event_ids_are_suppressed() {
    ...
    let envelope = CampaignStarted { campaign_id: "cmp-003".into() }
        .into_envelope("affiliate-engine")
        .unwrap();

    assert_eq!(bus.publish(envelope.clone()).unwrap(), PublishOutcome::Published { handlers_called: 1 });
    assert_eq!(bus.publish(envelope).unwrap(), PublishOutcome::DuplicateSuppressed);
...
fn unsubscribe_removes_handler() {
    ...
    let envelope = CampaignStarted { campaign_id: "cmp-004".into() }
        .into_envelope("affiliate-engine")
        .unwrap();

    assert_eq!(bus.publish(envelope).unwrap(), PublishOutcome::Published { handlers_called: 0 });
    assert_eq!(*calls.lock().unwrap(), 0);
}

#[test]
fn envelope_serialization_round_trip_preserves_contract() {
    let envelope = CampaignStarted { campaign_id: "cmp-005".into() }
        .into_envelope("affiliate-engine")
        .unwrap();
