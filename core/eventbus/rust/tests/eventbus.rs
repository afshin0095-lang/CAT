...
impl CatEvent for CampaignStarted { const TYPE: &'static str = "affiliate.campaign.started"; const VERSION: u16 = 1; }
...
fn handlers_run_in_registration_order() {
    ...
    for marker in [1_u8, 2_u8, 3_u8] {
        ...
        bus.subscribe(CampaignStarted::TYPE, Arc::new(move |_| { observed.lock().unwrap().push(marker); Ok(()) })).unwrap();
    }
    let envelope = CampaignStarted { campaign_id: "cmp-002".into() }.into_envelope("affiliate-engine").unwrap();
    assert_eq!(bus.publish(envelope).unwrap(), PublishOutcome::Published { handlers_called: 3 });
