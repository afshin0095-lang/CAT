...
fn database_url() -> Option<String> {
    std::env::var("CAT_TEST_DATABASE_URL").ok()
}
...
async fn checkpoint_store_is_monotonic_and_idempotent() {
    ...
    sqlx::migrate!("./migrations")
        .run(&pool)
        .await
        .expect("apply CAT event-store migrations");
    ...
    store
        .save(&conflicting)
        .await
        .expect("equal-sequence conflict must be safely ignored");
    ...
    sqlx::query(
        "DELETE FROM cat_projection_checkpoints WHERE projection_id = $1 AND stream_id = $2",
    )
    .bind(&projection_id)
    .bind(stream_id.as_uuid())
    .execute(&pool)
    .await
    .expect("cleanup checkpoint contract row");
}
...
async fn checkpoint_store_supports_independent_projection_stream_pairs() {
    ...
    let loaded_a = store
        .load(&projection_a, stream_a)
        .await
        .expect("load projection A")
        .expect("projection A exists");
    let loaded_b = store
        .load(&projection_b, stream_b)
        .await
    ...
    sqlx::query(
        "DELETE FROM cat_projection_checkpoints WHERE (projection_id, stream_id) IN (($1,$2),($3,$4))",
    )
    .bind(&projection_a)
    .bind(stream_a.as_uuid())
    .bind(&projection_b)
    .bind(stream_b.as_uuid())
    .execute(&pool)
    .await
    .expect("cleanup checkpoint isolation rows");
}
