...
async fn inbox_isolated_by_consumer_and_idempotent_per_consumer() {
    ...
    assert_eq!(first.state(event_id).await.unwrap(), Some(DeliveryState::Succeeded));
    assert_eq!(second.state(event_id).await.unwrap(), Some(DeliveryState::RetryScheduled));
}
