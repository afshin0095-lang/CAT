...
fn publisher_defaults_are_conservative_for_recovery_and_polling() {
    ...
    assert_eq!(config.retry_policy.delay_for(1), Duration::from_millis(250));
}
...
#[test]
fn terminal_outcome_preserves_attempt_count() {
    let event_id = Uuid::now_v7();
    let outcome = PublicationOutcome::DeadLettered { event_id, attempts: 5 };
    assert_eq!(outcome, PublicationOutcome::DeadLettered { event_id, attempts: 5 });
}
