...
///
/// The worker intentionally acknowledges the database row only after the
/// transport has acknowledged publication. This gives CAT an at-least-once
/// publication boundary; the event id remains the deduplication identity on the
/// transport side.
    ...
    T: AsyncEventTransport,
{
    ...
    pub async fn publish_one(&self) -> EventBusResult<PublicationOutcome> {
        ...
        match self.transport.publish(&record.event).await {
            ...
            Err(error) => {
                let state = self
                    .outbox
                    .fail(event_id, attempts, &self.config.retry_policy, &error.to_string())
                    .await?;

                match state {
                    DeliveryState::DeadLettered => {
                        Ok(PublicationOutcome::DeadLettered { event_id, attempts })
                    }
    ...
    ///
    /// A stale in-flight claim is recovered before each publication attempt so
    /// a worker crash cannot permanently strand an outbox record.
