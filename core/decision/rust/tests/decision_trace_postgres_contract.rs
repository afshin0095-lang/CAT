use cat_decision::{
    Alternative, DecisionOutcome, DecisionStatus, DecisionTrace, DurableDecisionTraceEventStore,
};
use cat_eventstore_postgres::PostgresEventStore;
use cat_kernel::{
    CorrelationId, EntityId, ExpectedVersion, FixedClock, IdempotencyKey, TenantId, TimestampMs,
};
use serde_json::json;
use uuid::Uuid;
...
async fn durable_trace_round_trip_preserves_advisory_replay() {
    ...
    adapter
        .persist(
            ...
            &IdempotencyKey::new(format!("decision-durable-{}", Uuid::now_v7())).unwrap(),
            &clock,
        )
        .await
        .unwrap();
...
async fn durable_trace_rejects_stale_version() {
    ...
    adapter
        .persist(
            ...
            CorrelationId::new(),
            ExpectedVersion::Empty,
            &IdempotencyKey::new(format!("decision-durable-a-{}", Uuid::now_v7())).unwrap(),
            &clock,
        )
        .await
        .unwrap();
    ...
    assert!(
        adapter
            .persist(
                ...
                CorrelationId::new(),
                ExpectedVersion::Empty,
                &IdempotencyKey::new(format!("decision-durable-b-{}", Uuid::now_v7())).unwrap(),
                &clock,
            )
            .await
            .is_err()
