use cat_decision::{Alternative, DecisionOutcome, DecisionStatus, DecisionTrace, DecisionTraceEventStore};
use cat_kernel::{CorrelationId, ExpectedVersion, FixedClock, IdempotencyKey, TenantId, TimestampMs};
use serde_json::json;
use uuid::Uuid;
...
fn persisted_trace_hydrates_and_replays_without_execution_authority() {
    ...
    store.persist(trace, TenantId::new(), CorrelationId::new(), ExpectedVersion::Empty, IdempotencyKey::new("decision-trace-1").unwrap(), &clock).unwrap();
...
fn stale_expected_version_fails_closed() {
    ...
    store.persist(trace.clone(), TenantId::new(), CorrelationId::new(), ExpectedVersion::Empty, IdempotencyKey::new("trace-a").unwrap(), &clock).unwrap();
    assert!(store.persist(trace, TenantId::new(), CorrelationId::new(), ExpectedVersion::Empty, IdempotencyKey::new("trace-b").unwrap(), &clock).is_err());
}
