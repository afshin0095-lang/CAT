...
pub mod metric_names {
    ...
    pub const OPPORTUNITY_REVALIDATION_REQUESTED: &str = "affiliate.opportunity.revalidation_requested";
    pub const OPPORTUNITY_REVALIDATION_SUCCEEDED: &str = "affiliate.opportunity.revalidation_succeeded";
...
impl MetricSample {
    pub fn counter(name: &'static str, value: u64) -> Self {
        Self { name, value, labels: Vec::new() }
    }
...
pub fn ingestion_report_metrics(report: &OpportunityIngestionReport) -> Vec<MetricSample> {
    ...
    vec![
        MetricSample::counter(metric_names::DISCOVERY_CANDIDATES_RECEIVED, report.candidates_received as u64)
            .with_label(label_keys::BATCH_ID, batch_label.clone()),
        MetricSample::counter(metric_names::DISCOVERY_CANDIDATES_REJECTED, report.candidates_rejected as u64)
            .with_label(label_keys::BATCH_ID, batch_label.clone()),
        MetricSample::counter(metric_names::DISCOVERY_OPPORTUNITIES_CREATED, report.opportunities_created as u64)
            .with_label(label_keys::BATCH_ID, batch_label.clone()),
        MetricSample::counter(metric_names::DISCOVERY_OPPORTUNITIES_CHANGED, report.opportunities_changed as u64)
            .with_label(label_keys::BATCH_ID, batch_label.clone()),
        MetricSample::counter(metric_names::DISCOVERY_SOURCE_FAILURES, report.sources_failed as u64)
...
pub fn revalidation_status_metrics(status: RevalidationStatus, count: u64) -> Vec<MetricSample> {
    match status {
        RevalidationStatus::Pending | RevalidationStatus::Claimed | RevalidationStatus::Running => {
            vec![MetricSample::counter(metric_names::OPPORTUNITY_REVALIDATION_REQUESTED, count)]
        }
        RevalidationStatus::Succeeded => {
            vec![MetricSample::counter(metric_names::OPPORTUNITY_REVALIDATION_SUCCEEDED, count)]
        }
        RevalidationStatus::Failed | RevalidationStatus::DeadLettered => {
            vec![MetricSample::counter(metric_names::OPPORTUNITY_REVALIDATION_FAILED, count)]
        }
...
mod tests {
    ...
    use crate::opportunity_lifecycle::{LifecycleEvaluation, LifecycleStateCounts, OpportunityLifecyclePolicy, OpportunityLifecycleSnapshot};
    use uuid::Uuid;
    ...
    fn revalidation_statuses_map_to_the_correct_metric_family() {
        ...
        assert_eq!(requested[0].name, metric_names::OPPORTUNITY_REVALIDATION_REQUESTED);
        assert_eq!(requested[0].value, 4);

        let succeeded = revalidation_status_metrics(RevalidationStatus::Succeeded, 2);
        assert_eq!(succeeded[0].name, metric_names::OPPORTUNITY_REVALIDATION_SUCCEEDED);

        let failed = revalidation_status_metrics(RevalidationStatus::DeadLettered, 1);
        assert_eq!(failed[0].name, metric_names::OPPORTUNITY_REVALIDATION_FAILED);
    ...
    fn in_memory_sink_aggregates_totals_deterministically() {
        ...
        sink.record(MetricSample::counter(metric_names::DISCOVERY_CANDIDATES_RECEIVED, 3));
        sink.record(MetricSample::counter(metric_names::DISCOVERY_CANDIDATES_RECEIVED, 4).with_label("source", "network-a"));
        sink.record(MetricSample::counter(metric_names::DISCOVERY_SOURCE_FAILURES, 1));
        let totals = sink.totals();
        assert_eq!(totals.get(metric_names::DISCOVERY_CANDIDATES_RECEIVED), Some(&7));
        assert_eq!(totals.get(metric_names::DISCOVERY_SOURCE_FAILURES), Some(&1));
