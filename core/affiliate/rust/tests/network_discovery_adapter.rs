use cat_affiliate::{
    AffiliateDomainResult, DiscoverySource, DiscoverySourceRequest, NetworkAdapter,
    NetworkConversion, NetworkDiscoveryAdapter, NetworkId, NetworkInfo, NetworkProgram,
};
...
impl NetworkAdapter for StubNetwork {
    ...
    fn list_programs(
        ...
        Box::pin(async move { Ok(programs) })
    }
    fn generate_link(
        &self,
        _: &str,
        ...
        Box::pin(async { Ok("https://example.invalid/link".into()) })
    }
...
fn network(programs: Vec<NetworkProgram>) -> StubNetwork {
    StubNetwork {
        info: NetworkInfo {
            id: NetworkId("net-a".into()),
            name: "Network A".into(),
            base_url: "https://network.invalid".into(),
...
#[tokio::test]
async fn network_programs_are_ingested_as_candidates() {
...
#[tokio::test]
async fn unsupported_filters_fail_before_network_call() {
