...
use cat_affiliate::{
    ...
    OpportunityHealth, OpportunityRecord, OpportunityRevision, OpportunityStatusProjector, OpportunityStore,
    RevalidationBlockReason,
};
use uuid::Uuid;
...
fn view_tracks_lifecycle_transitions_end_to_end() {
    let projector = OpportunityStatusProjector::new(FreshnessPolicy::new(1_000, 5_000, 9_000).expect("valid policy"));
...
fn view_exposes_revision_for_optimistic_concurrency() {
    ...
    let opportunity = DiscoveryOpportunity { id: Uuid::now_v7(), candidate, score: 5_000, rank: 1 };
...
fn blocked_revalidation_is_visible_in_the_view() {
    ...
    assert!(view.revalidation_reasons.is_empty(), "no request could be planned");
    assert_eq!(view.revalidation_blocked, Some(RevalidationBlockReason::AllSourcesUnavailable));
}
...
fn views_serialize_for_read_model_transport() {
    ...
    let decoded: cat_affiliate::OpportunityStatusView = serde_json::from_str(&encoded).expect("deserialize");
