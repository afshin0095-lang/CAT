...
use cat_affiliate::{
    InMemorySourceHealthStore, SourceHealthState, SourceHealthStore,
};
...
fn health_journey_unknown_to_healthy_to_unavailable_to_healthy() {
    ...
    assert_eq!(snapshot.total_failures, u64::from(cat_affiliate::UNAVAILABLE_AFTER_CONSECUTIVE_FAILURES));
...
fn multiple_sources_are_tracked_independently() {
    ...
    let sources: Vec<&str> = store
        ...
        .collect();
    assert_eq!(sources, vec!["alpha", "beta"], "iteration order is deterministic");
}
...
fn snapshots_are_plain_data() {
    ...
    let decoded: cat_affiliate::SourceHealthSnapshot = serde_json::from_str(&encoded).expect("deserialize");
