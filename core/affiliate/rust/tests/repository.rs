use cat_affiliate::{AffiliateDomain, AffiliateKind, AffiliateRepository, InMemoryAffiliateRepository};
...
fn full_affiliate_flow_can_be_persisted_without_mutating_truth() {
    ...
    assert_eq!(repo.offer(offer.id).unwrap().status, cat_affiliate::OfferStatus::Active);
}
