...
use cat_affiliate::{
    canonical_key, DiscoveryCandidate, DiscoveryOpportunity, FreshnessPolicy, FreshnessState, InMemoryOpportunityStore,
    OpportunityFilter, OpportunityQuery, OpportunityQueryService, OpportunitySort, OpportunitySortField,
    OpportunityStore, SortDirection,
};
use uuid::Uuid;
...
fn store_with(opportunities: Vec<cat_affiliate::DiscoveryOpportunity>) -> Vec<cat_affiliate::OpportunityRecord> {
...
fn end_to_end_filter_sort_and_paginate() {
    ...
    let service = OpportunityQueryService::new(FreshnessPolicy::new(1_000, 5_000, 9_000).expect("valid policy"));

    let mut filter = OpportunityFilter::default();
    filter.merchant = Some("Beta".into());
    let query = OpportunityQuery::new(filter).with_limit(1);
    let page = service.execute(&records, &query, 12_500).expect("valid query");
    ...
    let page = service.execute(&records, &query, 12_500).expect("valid query");
    assert_eq!(page.items[0].identity, "beta:gamma", "most recently observed first");
}
...
fn lifecycle_filter_uses_the_derived_state_not_storage() {
    ...
    let result = service.execute(&records, &query, 15_500).expect("valid query");
    assert_eq!(result.total_matched, 1);
    assert_eq!(result.items[0].identity, "acme:old", "5.5s-old observation is stale with a 5s stale threshold");
}
