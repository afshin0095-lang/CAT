...
use crate::{MemoryId, MemoryObject, MemoryPolicy, MemoryOperation, MemoryStoreError};
...
impl InMemoryMemoryStore {
    pub fn new(policy: MemoryPolicy, now_ms: u64) -> Self {
        Self { objects: BTreeMap::new(), policy, now_ms }
    }
...
impl MemoryStore for InMemoryMemoryStore {
    ...
    fn remove(&mut self, id: MemoryId) -> Result<MemoryObject, MemoryStoreError> {
        self.objects.remove(&id).ok_or(MemoryStoreError::NotFound(id))
    }

    fn list_namespace(&self, namespace: &str) -> Vec<&MemoryObject> {
        self.objects.values().filter(|item| item.namespace == namespace).collect()
    }
}
