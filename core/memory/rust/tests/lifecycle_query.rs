use cat_memory::{
    Classification, Consent, LifecycleState, MemoryId, MemoryKind, MemoryObject, MemoryQuery,
    Provenance, Retention, filter, is_terminal, validate_transition,
};
...
fn query_filters_memory_objects_without_mutation() {
    ...
    let matches = filter(
        objects.iter(),
        MemoryQuery::namespace("affiliate")
            .with_kind(MemoryKind::Semantic)
            .with_state(LifecycleState::Active),
    );
...
fn query_respects_classification_floor() {
    ...
    let matches = filter(
        objects.iter(),
        MemoryQuery::all().with_minimum_classification(Classification::Confidential),
    );
    assert_eq!(matches.len(), 1);
