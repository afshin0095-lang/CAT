...
use cat_orchestrator::{
    StepState, WorkflowCompleted, WorkflowEventFactory, WorkflowStarted, WorkflowState,
...
fn workflow_event_contracts_have_stable_names_and_versions() {
    ...
    assert_eq!(
        WorkflowStepStateChanged::TYPE,
        "orchestrator.workflow.step.state_changed"
    );
...
fn started_event_preserves_workflow_identity_and_contract_metadata() {
    ...
    let envelope =
        WorkflowEventFactory::started(workflow_id, "affiliate.settlement", 3, "orchestrator")
            .unwrap();
...
fn completed_event_is_explicitly_terminal_and_versioned() {
    let workflow_id = Uuid::now_v7();
    let envelope =
        WorkflowEventFactory::completed(workflow_id, WorkflowState::Succeeded, 9, "orchestrator")
            .unwrap();
