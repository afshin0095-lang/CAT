...
/// Deterministic workflow state transitions for the orchestrator's execution boundary.
///
/// The engine intentionally contains no transport, storage, or worker implementation.
/// Those concerns remain behind the platform/eventbus ports so the state machine can be
...
impl ExecutionEngine {
    ...
    pub fn refresh_ready_steps(&self, workflow: &mut WorkflowInstance) -> OrchestratorResult<()> {
        ...
        let succeeded: HashSet<String> = workflow
            .definition
            .steps
            .iter()
        ...
        for step in &mut workflow.definition.steps {
            ...
            if step.dependencies.iter().all(|dependency| succeeded.contains(dependency)) {
        ...
        if workflow
            ...
            .steps
            .iter()
            .any(|step| step.state == StepState::Ready || step.state == StepState::Running)
        {
            workflow.state = WorkflowState::Running;
        } else if workflow.definition.steps.iter().all(|step| {
            matches!(step.state, StepState::Succeeded | StepState::Compensated | StepState::Skipped)
        }) {
    ...
    pub fn begin_step(&self, workflow: &mut WorkflowInstance, step_id: &str) -> OrchestratorResult<()> {
        let step = workflow
            ...
            .ok_or_else(|| OrchestratorError::UnknownStep { step_id: step_id.to_owned() })?;

        if step.state != StepState::Ready {
            return Err(OrchestratorError::InvalidStepTransition {
                step_id: step_id.to_owned(),
                from: format!("{:?}", step.state),
                to: "running".into(),
            });
        }
