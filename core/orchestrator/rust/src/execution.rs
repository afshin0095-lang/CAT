...
impl ExecutionEngine {
    pub fn start(&self, workflow: &mut WorkflowInstance) -> OrchestratorResult<()> {
        if workflow.state != WorkflowState::Pending {
            return Err(OrchestratorError::InvalidStateTransition {
                from: format!("{:?}", workflow.state), to: "running".into(),
            });
        }
    ...
    pub fn refresh_ready_steps(&self, workflow: &mut WorkflowInstance) -> OrchestratorResult<()> {
        ...
        let succeeded: HashSet<String> = workflow.definition.steps.iter()
            .filter(|step| step.state == StepState::Succeeded)
            .map(|step| step.id.clone()).collect();

        for step in &mut workflow.definition.steps {
            if step.state == StepState::Pending && step.dependencies.iter().all(|dependency| succeeded.contains(dependency)) {
    ...
    pub fn begin_step(&self, workflow: &mut WorkflowInstance, step_id: &str) -> OrchestratorResult<()> {
        let step = workflow.definition.steps.iter_mut().find(|step| step.id == step_id)
