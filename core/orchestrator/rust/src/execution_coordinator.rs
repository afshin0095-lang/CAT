...
use crate::{
    decide_retry, DurableWorkflowStore, ExecutionEngine, ExecutionEventSink, LeaseProvider,
    OrchestratorResult, RetryPolicy, StepState, WorkerExecutionInput, WorkerExecutionOutcome,
    WorkerExecutor, WorkflowEventFactory,
};
...
{
    pub fn execute_step(&mut self, workflow_id: Uuid, step_id: &str, now_ms: u64) -> OrchestratorResult<crate::DispatchResult> {
        let mut workflow = self.store.load(workflow_id)?;
        let expected_revision = workflow.revision;
        let step = workflow.definition.steps.iter().find(|step| step.id == step_id)
        ...
        let result = self.worker.execute(WorkerExecutionInput::from_request(execution_id, &request));
        let retry = matches!(result.outcome, WorkerExecutionOutcome::Failed).then(|| decide_retry(self.retry_policy, request.attempt));
        ...
        match dispatch.action {
            ...
            crate::DispatchAction::Retry { .. } | crate::DispatchAction::WaitForApproval => self.engine.wait_step(&mut workflow, step_id)?,
        ...
        let final_state = workflow.definition.steps.iter().find(|step| step.id == step_id).map(|step| step.state).unwrap_or(StepState::Failed);
        ...
        self.store.commit(workflow, expected_revision, std::slice::from_ref(&event))?;
...
mod tests {
    use super::*;
    use crate::{InMemoryDurableWorkflowStore, InMemoryLeaseProvider, RecordingExecutionEventSink, WorkflowDefinition, WorkflowInstance, WorkflowStep, WorkflowState, WorkerExecutionResult};

    struct SuccessWorker;
    impl WorkerExecutor for SuccessWorker {
        fn execute(&mut self, _input: WorkerExecutionInput) -> WorkerExecutionResult { WorkerExecutionResult::success(serde_json::json!({"ok": true})) }
    }

    fn ready_workflow() -> WorkflowInstance {
        WorkflowInstance {
            id: Uuid::now_v7(), state: WorkflowState::Running, revision: 0,
    ...
    fn coordinator_commits_worker_success() {
        ...
        let workflow = ready_workflow(); let id = workflow.id; store.insert(workflow);
        let mut leases = InMemoryLeaseProvider::default(); let mut worker = SuccessWorker; let mut events = RecordingExecutionEventSink::default();
        let mut coordinator = ExecutionCoordinator { store: &mut store, leases: &mut leases, worker: &mut worker, events: &mut events, engine: ExecutionEngine::default(), retry_policy: RetryPolicy::default(), owner: "worker-1".into(), lease_ttl_ms: 10_000 };
