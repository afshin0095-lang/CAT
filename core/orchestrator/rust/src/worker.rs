...
impl WorkerExecutionResult {
    pub fn success(output: serde_json::Value) -> Self {
        Self { outcome: WorkerExecutionOutcome::Succeeded, output, error_code: None }
    }

    pub fn failure(error_code: impl Into<String>, output: serde_json::Value) -> Self {
        Self { outcome: WorkerExecutionOutcome::Failed, output, error_code: Some(error_code.into()) }
    }
}

/// Side-effect boundary for workers that execute CAT step requests.
/// Implementations own the actual tool/platform calls; the orchestrator only
/// supplies an explicit request and consumes the returned result.
...
mod tests {
    ...
    #[test]
    fn request_maps_to_worker_input_without_losing_attempt() {
    ...
    #[test]
    fn result_helpers_preserve_machine_readable_outcomes() {
        let success = WorkerExecutionResult::success(serde_json::json!({"count": 2}));
        assert_eq!(success.outcome, WorkerExecutionOutcome::Succeeded);
        assert!(success.error_code.is_none());

        let failure = WorkerExecutionResult::failure("provider_timeout", serde_json::json!({}));
        assert_eq!(failure.outcome, WorkerExecutionOutcome::Failed);
        assert_eq!(failure.error_code.as_deref(), Some("provider_timeout"));
    }
}
