...
impl ExecutionEventSink for EventBusExecutionEventSink<'_> {
    fn publish(&mut self, event: EventEnvelope) -> OrchestratorResult<()> {
        self.bus.publish(event).map(|_| ()).map_err(|error| OrchestratorError::Serialization(error.to_string()))
    }
}
...
impl InMemoryDurableWorkflowStore {
    pub fn insert(&mut self, workflow: WorkflowInstance) { self.workflows.insert(workflow.id, workflow); }
    pub fn outbox(&self) -> &[EventEnvelope] { &self.outbox }
}

impl DurableWorkflowStore for InMemoryDurableWorkflowStore {
    fn load(&self, workflow_id: Uuid) -> OrchestratorResult<WorkflowInstance> {
        self.workflows.get(&workflow_id).cloned().ok_or_else(|| OrchestratorError::WorkflowNotFound(workflow_id.to_string()))
    }
    ...
    fn commit(&mut self, workflow: WorkflowInstance, expected_revision: u64, events: &[EventEnvelope]) -> OrchestratorResult<()> {
        match self.workflows.get(&workflow.id) {
            ...
            Some(current) => Err(OrchestratorError::RevisionConflict { workflow_id: workflow.id.to_string(), expected: expected_revision, actual: current.revision }),
...
#[derive(Default)]
pub struct InMemoryLeaseProvider { leases: HashMap<String, Lease> }
...
mod tests {
    ...
    fn workflow() -> WorkflowInstance {
        WorkflowInstance { id: Uuid::now_v7(), definition: WorkflowDefinition { workflow_type: "test".into(), version: 1, steps: vec![WorkflowStep { id: "work".into(), dependencies: vec![], state: StepState::Ready, attempt: 0, max_attempts: 3, compensation_step: None }] }, state: WorkflowState::Running, revision: 0 }
    }
    ...
    fn lease_is_exclusive_until_expiry() {
        let mut provider = InMemoryLeaseProvider::default(); provider.acquire("workflow/1", "worker-a", 100, 50).unwrap();
