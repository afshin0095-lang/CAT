...
pub trait DurableOutboxStore {
    fn enqueue(&mut self, event: EventEnvelope) -> OrchestratorResult<()>;
    fn claim_next(&mut self, owner: &str, now_ms: u64) -> OrchestratorResult<Option<OutboxRecord>>;
    fn acknowledge(&mut self, event_id: Uuid, owner: &str) -> OrchestratorResult<()>;
...
impl InMemoryDurableOutbox {
    pub fn pending(&self) -> usize { self.records.len() }
}

impl DurableOutboxStore for InMemoryDurableOutbox {
    fn enqueue(&mut self, event: EventEnvelope) -> OrchestratorResult<()> {
        if self.records.iter().any(|record| record.event.event_id == event.event_id) {
            return Err(OrchestratorError::Serialization("duplicate outbox event id".into()));
        }
        self.records.push_back(OutboxRecord { event, attempt: 0, available_at_ms: 0, claimed_by: None });
        Ok(())
    }
    ...
    fn claim_next(&mut self, owner: &str, now_ms: u64) -> OrchestratorResult<Option<OutboxRecord>> {
        ...
        record.claimed_by = Some(owner.to_owned());
        Ok(Some(record.clone()))
    }

    fn acknowledge(&mut self, event_id: Uuid, owner: &str) -> OrchestratorResult<()> {
        let position = self.records.iter().position(|record| record.event.event_id == event_id)
            .ok_or_else(|| OrchestratorError::Serialization("outbox event not found".into()))?;
        if self.records[position].claimed_by.as_deref() != Some(owner) {
    ...
    fn fail(&mut self, event_id: Uuid, owner: &str, now_ms: u64, policy: RetryPolicy, error: &str) -> OrchestratorResult<OutboxDisposition> {
        ...
        if policy.retryable(record.attempt) {
            ...
            let _ = error;
            self.records.remove(position);
            Ok(OutboxDisposition::DeadLettered { attempt: record.attempt })
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn failed_record_is_rescheduled_before_exhaustion() {
        let mut outbox = InMemoryDurableOutbox::default();
        let event = event(); let id = event.event_id; outbox.enqueue(event).unwrap();
        outbox.claim_next("dispatcher-a", 100).unwrap();
        let disposition = outbox.fail(id, "dispatcher-a", 100, RetryPolicy::default(), "timeout").unwrap();
        assert!(matches!(disposition, OutboxDisposition::RetryScheduled { .. }));
        assert!(outbox.claim_next("dispatcher-a", 100).unwrap().is_none());
    }

    #[test]
    fn claim_cannot_be_acked_by_wrong_owner() {
        let mut outbox = InMemoryDurableOutbox::default();
        let event = event(); let id = event.event_id; outbox.enqueue(event).unwrap(); outbox.claim_next("dispatcher-a", 0).unwrap();
        assert!(matches!(outbox.acknowledge(id, "dispatcher-b"), Err(OrchestratorError::LeaseOwnerMismatch { .. })));
    }
}
