    ...
    fn enqueue(&mut self, event: EventEnvelope) -> OrchestratorResult<()> {
            ...
            .any(|record| record.event.event_id == event.event_id)
        {
            return Err(OrchestratorError::Serialization(
                "duplicate outbox event id".into(),
            ));
        }
        self.records.push_back(OutboxRecord {
            event,
            attempt: 0,
            available_at_ms: 0,
            claimed_by: None,
        ...
        let Some(position) = self.records.iter().position(|record| {
            record.available_at_ms <= now_ms
                && record
                    .claimed_by
                    .as_deref()
                    .is_none_or(|claimant| claimant == owner)
        }) else {
            return Ok(None);
        };
        let record = self
...
mod tests {
    ...
    fn event() -> EventEnvelope {
        EventEnvelope {
            ...
            event_type: "orchestrator.test".into(),
            version: 1,
    ...
    fn failed_record_is_rescheduled_before_exhaustion() {
        ...
        let disposition = outbox
            .fail(id, "dispatcher-a", 100, RetryPolicy::default(), "timeout")
            .unwrap();
        assert!(matches!(
            disposition,
    ...
    fn claim_cannot_be_acked_by_wrong_owner() {
        ...
        let id = event.event_id;
        outbox.enqueue(event).unwrap();
        outbox.claim_next("dispatcher-a", 0).unwrap();
        assert!(matches!(
            outbox.acknowledge(id, "dispatcher-b"),
            Err(OrchestratorError::LeaseOwnerMismatch { .. })
        ));
    }
}
