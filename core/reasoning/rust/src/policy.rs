use crate::{ReasoningError, ReasoningExecutionResult, ReasoningRequest};
...
impl ReasoningPolicy {
    pub fn validate(&self, request: &ReasoningRequest) -> ReasoningExecutionResult<()> {
        ...
        if request.max_steps == 0 || request.max_steps > self.max_steps {
            return Err(ReasoningError::PolicyRejected(format!("max_steps must be between 1 and {}", self.max_steps)));
        }
        for evidence in &request.evidence {
            if !(0.0..=1.0).contains(&evidence.confidence) { return Err(ReasoningError::InvalidConfidence); }
        }
        Ok(())
    }
}
