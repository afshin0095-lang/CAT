use crate::{Hypothesis, ReasoningError, ReasoningOutcome, ReasoningPolicy, ReasoningRequest, ReasoningResult, ReasoningStep};
...
impl DeterministicReasoningEngine {
    ...
    pub fn reason(&self, request: &ReasoningRequest) -> ReasoningOutcome<ReasoningResult> {
        ...
        let mut steps = Vec::new(); let mut hypotheses = Vec::new();
        ...
        for evidence in &request.evidence {
            ...
            entry.0 += if evidence.authoritative { evidence.confidence } else { evidence.confidence * 0.75 };
            entry.1 += 1.0 - evidence.confidence; entry.2.push(evidence.evidence_id);
        }
        for (statement, (support, contradiction, evidence_ids)) in support_by_statement {
            let confidence = if support + contradiction > 0.0 { support / (support + contradiction) } else { 0.0 };
        ...
        hypotheses.sort_by(|a, b| { let ca = a.support / (a.support + a.contradiction).max(f32::EPSILON); let cb = b.support / (b.support + b.contradiction).max(f32::EPSILON); cb.total_cmp(&ca).then_with(|| a.statement.cmp(&b.statement)) });
