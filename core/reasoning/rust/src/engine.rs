...
impl DeterministicReasoningEngine {
    ...
    pub fn reason(&self, request: &ReasoningRequest) -> ReasoningExecutionResult<ReasoningResult> {
        ...
        Ok(ReasoningResult {
            reasoning_id: Uuid::now_v7(), request_id: request.request_id,
            conclusion: format!("Best-supported hypothesis: {}", top.statement), confidence,
            hypotheses, steps, evidence_used: request.evidence.iter().map(|e| e.evidence_id).collect(),
            assumptions: vec!["Evidence weights are treated as independent signals.".into()], advisory_only: true,
        })
    }
}
